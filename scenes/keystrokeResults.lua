...
function scene:create( event )
    ...
    local timeInMS = event.params.milliseconds/event.params.letters
    local timerInSeconds = timeInMS / 1000

    local timeInMSText = display.newText("Time Per Keystroke(ms): " .. timeInMS, W * .1, H * .1, font, fontSize * .8 )
    timeInMSText.anchorX  = 0
    timeInMSText.anchorY = 0
    timeInMSText:setFillColor( 1, 1, 1 )

    local timeInSecondsTxt = display.newText("Time Per Keystroke(s): " .. timerInSeconds, W * .1, timeInMSText.y + 20, font, fontSize * .8 )
